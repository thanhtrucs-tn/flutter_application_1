...
The legacy namespaced entry points are now backed by user-invocable **skills** under `.claude/skills/`, not by a separate command parser. Each skill ships portable frontmatter (`name:`, `description:`, `user-invocable: true`) and is validated by CI gates (`check-skill-cross-refs.js`, `check-skill-descriptions.js`, `check-skill-routing.js`).

**Core Development Entry Points**:
- `plan` - Research and create implementation plans (`--deep` for major refactors, `--tdd` for tests-first plans)
- `cook` - Implement features with full workflow (`--tdd` for tests-first refactors, `--parallel` for multi-agent execution)
...
**Flow**:
1. Run `cook "add user authentication"` or `cook "refactor auth middleware" --tdd`
