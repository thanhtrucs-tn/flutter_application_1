...
> Note on architecture: as of the v2.17 release, the original namespaced slash-command layer has been fully migrated to user-invocable **skills** under `.claude/skills/`. References to "slash commands" in older sections of this roadmap describe routing entry points that are now skill-backed.
...
- **Commands → Skills migration** (v2.17): the namespaced slash-command surface is now skill-routed; `metadata.deletions[]` cleans stale command files on user upgrade
...
- **Review harness** (next): `fix`/`cook` move from score-only approval to artifact-gated review with high-risk auto stops
