...
- `plan` - Research and planning (`--deep` for major refactors, `--tdd` for tests-first plans)
- `cook` - Feature implementation (`--tdd` for tests-first refactors)
- `test` - Test execution
...
- **workflow-artifact-gate.cjs** - Validates `fix`/`cook` review artifacts before finalize/commit/ship-like boundaries. It supports soft warnings for finalize/commit and hard blocks for ship/push/PR/deploy when enabled.
