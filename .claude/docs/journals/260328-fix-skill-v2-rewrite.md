...
**Severity**: High
**Component**: `fix` skill (v1.2.0 → v2.0.0)
**Status**: Complete
...
3. Diagnose (not "debug") — structured analysis via `sequential-thinking` + `debug`
...
- `scout` — always first
- `debug` + `sequential-thinking` — always for diagnosis
- `problem-solving` — auto-triggers when 2+ hypotheses fail (was "conditional")
