...
**Core Development**:
- `plan` - Research and planning (`--deep` for large refactors, `--tdd` for tests-first plans)
...
**Opt-In Policy Hooks**:
- `workflow-artifact-gate.cjs` - Validates `fix`/`cook` review artifacts before finalize/commit/ship-like actions
