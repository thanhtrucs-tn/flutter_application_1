...
It automatically integrates with the workflow stack:
...
Teammates receive runtime context from the lead prompt and task instructions by default. The old SubagentStart context-injection hook (`team-context-inject.cjs`) is disabled unless explicitly opted in.
...
- Lead synthesizes findings into a single report at the runtime reports path
...
TaskCompleted event fires → task-completed-handler.cjs
    ...
    ├── Logs to {reports_path}/team-{name}-completions.md
