...
**IMPORTANT**: Analyze skills at `.claude1/skills/*` and activate as needed.

## Gemini Model Configuration

Read model from `.claude1/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf .claude1/.mcp.json .gemini/settings.json
...
```bash
npx tsx .claude1/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
