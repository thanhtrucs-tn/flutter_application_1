...
Visual verification of frontend implementations using `agent-browser`, `chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests.
...
- No real login/cookies needed: use `agent-browser`, Chrome MCP, or project-native browser tests.
- Real user login/cookies/profile needed: invoke `chrome-profile` and run:
...
**ok=false** → Follow the skill's setup playbook, or use `agent-browser` / project-native browser tests if profile state is not required.
