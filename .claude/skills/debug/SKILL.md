...
Visual verification of frontend implementations via `agent-browser`, `chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests. Detect if frontend-related → check browser tool availability → screenshot + console error check → report. Skip if not frontend.
...
- **Frontend:** `agent-browser`, `chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests for visual verification
...
**Typically follows:** `scout` (after locating relevant code)
**Typically precedes:** `fix` (fix the diagnosed issue), `brainstorm` (explore solutions for complex problems)
**Related:** `scout` (discover before debugging), `fix` (fix after diagnosing)
