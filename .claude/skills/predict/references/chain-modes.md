...
If a future use case demands invoking reason or probe **without** first running predict, the right move is to extract THIS file into standalone portable skills (`reason` or `probe` directory under `.claude/skills/`) at that time. Until then, folding keeps the catalog smaller and the user mental model cleaner.
