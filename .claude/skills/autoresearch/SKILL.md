...
related: [loop, predict, scenario, security]
metadata:
  author: claudekit
...
# autoresearch — Autoresearch Family Router
...
The upstream framework ships 11 sub-commands. Locally, 6 are absorbed (4 standalone skills + 2 folded as chain modes) and 5 are not yet absorbed. **For the core loop, use `loop`.**

| Upstream sub-command | Local skill | Status | Use when... |
|---|---|---|---|
| `/autoresearch` (core) | `loop` | Faithful | Improving a measurable metric (coverage, bundle size, perf) over N bounded iterations |
| `/autoresearch:predict` | `predict` | Faithful (in scope) | Multi-persona debate before risky changes; supports `--chain reason` and `--chain probe` (closed in #728) |
| `/autoresearch:scenario` | `scenario` | Faithful (in scope) | Edge-case generation across 12 dimensions; supports both one-shot and iterative saturation (closed in #729) |
| `/autoresearch:security` | `security` | Faithful (in scope) | STRIDE + OWASP audit with `--fix`; supports both one-shot and red-team-personas iterative discovery (closed in #730) |
| `/autoresearch:reason` | `predict --chain reason` | Folded | Subjective refinement loop — folded into `predict` chain modes rather than shipped as a standalone skill (closed in #728) |
| `/autoresearch:probe` | `predict --chain probe` | Folded | Requirement interrogation — folded into `predict` chain modes rather than shipped as a standalone skill (closed in #728) |
| `/autoresearch:plan` | — | Missing | Backfill candidate (HIGH priority) |
| `/autoresearch:debug` | — | Missing | Backfill candidate (HIGH priority) |
| `/autoresearch:fix` | — | Missing | Backfill candidate (MEDIUM); partly covered by `fix` |
| `/autoresearch:ship` | — | Missing | Backfill candidate (MEDIUM); partly covered by `ship` |
...
The exception: if you're authoring a new ClaudeKit skill that should adopt the autoresearch pattern (atomic commits, mandatory verify, guarded rollback), read this page + `loop`'s `references/` for the canonical implementation, then absorb only the relevant sub-command's workflow from upstream.
...
- Core iteration loop: `loop`
- Pre-implementation debate: `predict`
- Edge-case generation: `scenario`
- Security audit (uses autoresearch pattern): `security`
...
Earlier versions of this skill shipped as a byte-identical copy of `loop`'s body, marked "deprecated alias." That misrepresented the integration: locally the framework was already split, but the umbrella name absorbed nothing unique. As of 2026-05-02 (epic #711), this page is rewritten as a **concept anchor** so:

- `security`'s reference to "the autoresearch pattern" lands somewhere meaningful.
