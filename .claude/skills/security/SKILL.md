...
Runs a structured STRIDE + OWASP security audit on a given scope. Produces a severity-ranked findings report. With `--fix`, applies fixes iteratively using the autoresearch guard pattern.
...
| Audit + Fix | `security <scope> --fix` | Scan → categorize → fix iteratively |
| Red-team + Fix | `security <scope> --red-team --fix` | Full persona discovery → fix confirmed Critical/High |
...
- Feed Critical/High findings into `autoresearch --fix` for automated remediation
