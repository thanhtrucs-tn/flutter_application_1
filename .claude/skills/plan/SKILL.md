...
This skill orchestrates planning with a portable markdown-first workflow. If a compatible plan CLI is available, use it for scaffolding and phase state mutations; otherwise create and update `plan.md` plus `phase-*.md` files directly.

Compatible plan CLIs usually provide operations equivalent to:
...
cd /absolute/path/to/plan-dir && plan check <phase-id> --start
cd /absolute/path/to/plan-dir && plan check <phase-id>
cd /absolute/path/to/plan-dir && plan uncheck <phase-id>
plan status /absolute/path/to/plan.md
```

Rules:
- Use the plan CLI to scaffold `plan.md` and `phase-*.md` when available.
- Default scope is project-local (`./plans/` under the current project).
- Global scope is conditional: use the CLI's global option or fall back to global scope only when no project context exists.
- Use the plan CLI for phase status changes when available.
- If no plan CLI is available, edit markdown directly and preserve the table/frontmatter schema.
- Do not hand-edit the phases table for status toggles or structural updates when compatible CLI commands are available.
- **Generated-file write guard:** If a CLI creates existing `plan.md` and `phase-XX-*.md` stub files, read `plan.md` and **every generated phase stub** before composing long replacement content. A directory listing is not enough. Some tools enforce read-before-write on existing files; skipping any stub read can cause that file's write to be rejected after wasting the full write payload. The stubs are tiny, so read them all first, then fill them.

### Mandatory Generated-File Read Pass

After scaffold generation and before the first long Write/Edit to any generated plan file:
...
- Keep scope honest in prose and examples: the skill describes plan-file behavior, it does not implement scope resolution itself.
...
**Status interaction:** When a plan status command is available, use it as the authoritative inspection surface. Same-scope bare refs stay in the current scope; prefixed refs resolve against the explicit project/global root. Missing refs should warn and show `not found`, not hard-fail the plan.
