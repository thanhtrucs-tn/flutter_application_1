...
1. **Scaffold via CLI when available, otherwise create files directly:**
   ```bash
   plan create \
     --title "{plan title}" \
   ...
   plan create \
     ...
     [--issue {N}]
   ```
   If no compatible CLI exists, create `plan.md` and `phase-XX-*.md` using the templates below.
...
5. **Prefer CLI status commands when available.** If no compatible CLI exists, edit the Phases table and phase frontmatter directly while preserving schema and table structure.
...
- A compatible plan status command is the authoritative place to inspect resolved dependency state when available.
