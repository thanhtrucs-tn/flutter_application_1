#!/usr/bin/env python3
"""
Help - All-in-one guide with dynamic skill discovery.
Scans .claude/skills/ directory to build catalog at runtime.

Usage:
    python help.py                    # Overview with quick start
    python help.py fix                # Category guide with workflow
    python help.py plan validate      # Subcommand details
    python help.py debug login error  # Task recommendations
    python help.py auth               # Search (unknown word)
"""
...
def discover_skills(skills_dir: Path) -> dict:
    ...
    for skill_md in sorted(skills_dir.rglob("SKILL.md")):
        ...
        # Skip template and help itself
        if skill_name in ('template-skill', 'help'):
            continue
        ...
        # Clean description (remove legacy bracket prefixes and emoji indicators)
        clean_desc = re.sub(r'^\[[A-Z][A-Z0-9_-]*\]\s*', '', description).strip()
...
def show_overview(data: dict, prefix: str) -> None:
    ...
    print()
    print("**Quick Start:**")
    print("- `cook` - Implement features (standalone)")
    print("- `plan` + `cook` - Plan then execute")
    print("- `test` - Run and analyze tests")
    print()
    print("**Common Workflows:**")
    print("- New feature: `plan` -> `cook` -> `test`")
    print("- Review: `code-review` -> `watzup`")
    print()
    print("**Categories:**")
    ...
    print()
    print("**Tips:**")
    print("- Unclear about approach? -> `brainstorm` first")
    print("- Agent generated report? -> `preview` to view")
...
def show_config_guide() -> None:
    ...
    print()
    print("Guidelines auto-inject on session start. Skills like `brainstorm` respect them.")
    print()
    print("---")
    print()
...
def show_coding_level_guide() -> None:
    ...
    print("3. Skills like `brainstorm` follow the injected guidelines")
    print()
...
def main():
    ...
    # .claude/command-archive/help/scripts/help.py -> .claude/skills
    claude_root = script_path.parents[3]
